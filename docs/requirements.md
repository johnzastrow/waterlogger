...
# Changes for version 1.2 of the app. 

## New Feature 1: Calculating the volume. Calculating how much water is in a pool or hot tub


### Design: Provide this as an option on the pool setup screen to calculate volume. Then let user paste the value into the existing data field for volume.
Add new capability to the pools screen to calculate water volume based on choosing a pool shape and entering dimensions according to the formula for the pool shape. The app will perform the volume calculations for the selected shape using the user-provided dimensions and present a volume in gallons to screen, along with directions on the screen to paste the value into the existing pool volume field. The app should store the shape and dimensions in the database to allow the user to adjust and recalculate them later. Print the shape and dimension on the pool details screen if present in the database. The input units should be inches and the output units should be gallons.

### Background
1. Determine your pool volume
The first and most critical step is to accurately determine the volume of water your pool holds, typically measured in gallons. Here the units are feet. But switch them to inches for inputs.
...
## New Feature 2: Calculating Adjustments: how much chemical to add and in what order to correct water chemistry

### Develop a dedicated screen or screens launched from a button called "Recommended Changes" from the Sample detail screen to calculate what should be changed to set the water to have an ideal LSI and RSI. Put the same button on the dashboard where we show indices to use the latest sample measurements as inputs.

** consider the information in the file `chem_calcs.md` in this directory.

At the stop of the page display the pool details (name, size, type and any other) and the sample details (measurements from the water, date and time of sample) as a heading

Examine the approach takes by this [web page](https://www.troublefreepool.com/calc.html). The calculations should consider the starting value and then a target value for each chemical. Users should be able to adjust both the starting value and target values and then recalculate the needed additions when values are changed. 

Display a Starting RSI and LSI, and Target RSI and LSI and update these indices every time one of the values change.

For starting values (inputs) use the volume in the database for the selected pool where the sample was taken. Use the chemical sample values from the database as initial imputs, but let the user adjust the inputs using screen controls. Provide a 'Notes' field for the user to enter notes fore th

The logic and calculations appear to be in the Javascript of the web page itself. Reproduce the logic and screen contents, but use the additional requirements above as well. Do not reproduce the pool volume part of the page as this is addressed by the previous feature.

Also provide a 'Save Adjustments'  button on this screen. It should save a record with the following: date and time for the adjustment, notes from the user, all of the inputs (including starting and target values) and outputs (starting and target indices, recommended additions of each chemical). A new adjustment can be saved any time some value in the screen is changed.


● Looking at the requirements for version 1.2, I can see two new features to implement:

  1. Pool Volume Calculator - Add volume calculation capability to the pools screen based on shape and dimensions
  2. Chemical Adjustment Recommendations - Create a dedicated screen to calculate what chemicals to add to achieve ideal water
  chemistry

  I have a few clarifying questions before implementing these features:

  Pool Volume Calculator:
  1. Should the shape and dimensions be stored as additional fields in the existing POOLS table, or would you prefer a separate
  table for pool geometry? ANS: Use the existing POOLS table
  2. For the kidney-shaped pool formula, should I provide input fields for both "A" and "B" widths, or simplify with just one
  width field? ANS: both A and B widths, but be clear in the user interface when they are needed and when they are not.
  3. Should the volume calculator be a modal/popup on the pools page, or a separate section on the pool edit form? ANS: Let's try a modal popup.

  Chemical Adjustment Recommendations:
  1. I need to read the chem_calcs.md file you mentioned - is this in the docs/ directory? ANS: Yes
  2. For the "Save Adjustments" feature, should this create a new database table (like ADJUSTMENTS) to store the adjustment
  records? ANS: Yes, the ADJUSTMENTS table will have many ADJUSTMENTS 
  3. Should the adjustment records be linked to specific samples, or can they be standalone recommendations? ANS: They should link to the SAMPLE if the user does not change the STARTING inputs, which should be those values in the SAMPLE record
  4. What are the ideal target ranges for LSI and RSI that the system should aim for? ANS: LSI Ideal Range  = -0.3 to +0.3, best value is 0.0. RSI Ideal range is 5.0 to 7.0, best value is 6.0